...
import { fetchTrendingAction } from "../stores/actionCreator";
...
const Home = () => {
  ...
  const fetchTrending = (page) => {
    try {
      dispatcher(fetchTrendingAction(page));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const totalPages = trendingMovie?.total_pages || 0;
  const pageSetSize = 5;
    ...
    <div className="h-full w-full bg-gradient-to-b from-white to-gray-300 text-black flex justify-center overflow-auto">
      <div className="p-4 mx-auto flex flex-col justify-center w-full h-full">
        <div className="pb-8 pt-16">
          <p className="text-lg md:text-2xl lg:text-4xl text-gray-800 font-bold inline border-b-2 border-opacity-60 border-gray-500">Today Movie List</p>
        </div>
          ...
          <div className="flex justify-center mt-4">
            ...
            {getPageNumbersForSet(currentPageSet).map((page) => (
              <button
                ...
                className={`px-3 py-2 mx-1 rounded-lg ${page === currentPage ? "bg-indigo-500 text-white" : "bg-gray-300 text-gray-800 hover:bg-indigo-500 hover:text-white"}`}
              >
                {page}
              </button>
            ))}
