...
import { fetchSearchMoviesAction } from "../stores/actionCreator";
...
const Home = () => {
  ...
  const fetchSearchMovies = (page) => {
    try {
      dispatcher(fetchSearchMoviesAction(query, page));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const totalPages = searchMovies?.total_pages || 0;
  const pageSetSize = 5;
  ...
  return (
    <div className="h-full w-full bg-gradient-to-b from-white to-gray-300 text-gray-800 flex justify-center overflow-auto">
      <div className="p-4 mx-auto flex flex-col justify-start w-full h-screen">
        <div className="pb-8 pt-16">
          <p className="text-lg md:text-2xl lg:text-4xl text-gray-800 font-bold inline border-b-2 border-opacity-60 border-gray-500">Search result</p>
        </div>
